...
export function formatQuestObjectiveAtStep(
  ...
  if (total === 0) {
    return "Talk to a survivor to finish this quest.";
  }
  if (stepIndex >= total) {
    return "All objectives done — talk to a survivor to turn in.";
  }
  ...
  switch (step.type) {
    ...
    case "reach_waypoint":
      return `Reach the waypoint (tile ${step.row}, ${step.col}).`;
    case "kill_enemies": {
