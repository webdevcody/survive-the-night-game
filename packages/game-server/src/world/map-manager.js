...
import { CAMPSITE, FOREST1, FOREST2, FOREST3, WATER, FARM, GAS_STATION, CITY, MERCHANT, FOREST4, DOCK, SHED, } from "@/world/biomes";
...
export class MapManager {
    constructor() {
        ...
        this.campsiteFireTileX = Math.floor(MAP_SIZE / 2) * BIOME_SIZE + CAMPSITE_CAMPFIRE_LOCAL_X;
        this.campsiteFireTileY = Math.floor(MAP_SIZE / 2) * BIOME_SIZE + CAMPSITE_CAMPFIRE_LOCAL_Y;
     ...
     */
    selectCampsiteSurroundingBiomeSpawnLocations(campsiteBiomeX, campsiteBiomeY) {
    ...
    selectZombieSpawnLocations(count, centerBiomeX, centerBiomeY) {
        const validBiomes = [];
     ...
     */
    spawnZombieAtLocation(location, zombieType) {
     ...
     */
    spawnZombiesAroundCampsite(zombieType, count) {
        let spawnLocations = this.selectCampsiteSurroundingBiomeSpawnLocations(this.campsiteBiomeX, this.campsiteBiomeY);
        ...
        if (zombiesToSpawn < count) {
            console.warn(`Could not spawn all ${count} zombies around campsite. Only ${zombiesToSpawn} valid positions available.`);
        }
    }
    generateEmptyMap(width, height) {
        this.getEntityManager().clear();
        this.getEntityManager().setMapSize(width * getConfig().world.TILE_SIZE, height * getConfig().world.TILE_SIZE);
    ...
    generateSpatialGrid() {
        this.getEntityManager().setMapSize(BIOME_SIZE * MAP_SIZE * getConfig().world.TILE_SIZE, BIOME_SIZE * MAP_SIZE * getConfig().world.TILE_SIZE);
    }
    initializeMap() {
     ...
     */
    resolveCampsiteBiomeFromDecals() {
        var _a;
        ...
        outer: for (let y = 0; y < n; y++) {
            for (let x = 0; x < n; x++) {
                if (((_a = this.decalsLayer[y]) === null || _a === void 0 ? void 0 : _a[x]) === DECAL_TILE_CAMPSITE) {
     ...
     */
    getPlayerSpawnPositionForMap() {
        var _a;
        if (!this.authoredWorldMapApplied) {
            return (_a = this.getRandomCampsitePosition()) !== null && _a !== void 0 ? _a : this.getRandomGrassPosition();
        }
        const pos = this.getRandomSpawnLayerPlayerPosition();
        if (pos) {
            return pos;
        }
        console.warn("MapManager: authored world map has no valid player spawn tiles (spawns layer id 1 with empty collidables, not overlapping car).");
     ...
     */
    tryGetPositionForSavedTile(tileX, tileY) {
        var _a;
        const n = BIOME_SIZE * MAP_SIZE;
        ...
        if (((_a = this.collidablesLayer[ty]) === null || _a === void 0 ? void 0 : _a[tx]) !== EMPTY_COLLIDABLE_TILE_ID) {
            return null;
        }
    ...
    getRandomSpawnLayerPlayerPosition() {
        const n = BIOME_SIZE * MAP_SIZE;
        const TILE_SIZE = getConfig().world.TILE_SIZE;
        const poolManager = PoolManager.getInstance();
    ...
    seedOpenWorldZombieSpawnPointsFromAuthoredLayer() {
        const totalSize = BIOME_SIZE * MAP_SIZE;
        ...
        if (!anyEnemyMarker) {
            console.warn("MapManager: authored world map has no enemy spawn tiles (spawns layer ids 2–6); open-world zombie fixtures are disabled.");
        }
    }
    seedItemSpawnPointsFromAuthoredLayer() {
        const totalSize = BIOME_SIZE * MAP_SIZE;
        for (let y = 0; y < totalSize; y++) {
            for (let x = 0; x < totalSize; x++) {
                const id = this.spawnLayer[y][x];
    ...
    seedDialogueSurvivorNpcsFromAuthoredLayer() {
        const totalSize = BIOME_SIZE * MAP_SIZE;
        const byKey = new Map();
        ...
        for (let y = 0; y < totalSize; y++) {
            for (let x = 0; x < totalSize; x++) {
                ...
                const npc = new DialogueSurvivorNpc(this.getGameManagers(), entry !== null && entry !== void 0 ? entry : fallback, x, y);
                this.getEntityManager().addEntity(npc);
            }
        }
    }
    /** Procedural campsite: fire at legacy local tile (8,7) inside the biome. */
    spawnCampsiteFireAtBiome(biomeX, biomeY) {
    ...
    spawnCampsiteFireAtTile(tileX, tileY) {
        const campsiteFire = new CampsiteFire(this.getGameManagers());
        campsiteFire
            .getExt(Positionable)
            .setPosition(PoolManager.getInstance().vector2.claim(tileX * getConfig().world.TILE_SIZE, tileY * getConfig().world.TILE_SIZE));
        this.getEntityManager().addEntity(campsiteFire);
    }
    spawnLightDecalEntitiesFromDecalsLayer() {
        var _a;
        const n = BIOME_SIZE * MAP_SIZE;
        const TILE_SIZE = getConfig().world.TILE_SIZE;
        const poolManager = PoolManager.getInstance();
        for (let y = 0; y < n; y++) {
            for (let x = 0; x < n; x++) {
                if (((_a = this.decalsLayer[y]) === null || _a === void 0 ? void 0 : _a[x]) !== DECAL_TILE_LIGHT) {
                    continue;
                }
    ...
    spawnMessageDecalEntitiesFromDecalsLayer() {
        var _a;
        const byKey = new Map();
        ...
        for (let y = 0; y < n; y++) {
            for (let x = 0; x < n; x++) {
                if (((_a = this.decalsLayer[y]) === null || _a === void 0 ? void 0 : _a[x]) !== DECAL_TILE_MESSAGE) {
                    continue;
                }
                const entry = byKey.get(`${y},${x}`);
                const fallback = { row: y, col: x, lines: ["Read me."] };
                const lines = getMessageDecalLines(entry !== null && entry !== void 0 ? entry : fallback);
     ...
     */
    isNearCampsite(biomeX, biomeY) {
        const distance = Math.abs(biomeX - this.campsiteBiomeX) + Math.abs(biomeY - this.campsiteBiomeY);
     ...
     */
    isAdjacentToSpecialBiome(biomeX, biomeY, specialBiomes) {
     ...
     */
    selectRandomBiomePosition(excludedPositions) {
        const validPositions = [];
        ...
        for (let biomeY = 1; biomeY < MAP_SIZE - 1; biomeY++) {
            for (let biomeX = 1; biomeX < MAP_SIZE - 1; biomeX++) {
                ...
                const isExcluded = excludedPositions.some((pos) => pos && pos.x === biomeX && pos.y === biomeY);
                if (isExcluded) {
                    continue;
                }
                ...
                if (collidableTileId !== EMPTY_COLLIDABLE_TILE_ID) {
                    if (carTiles.has(collidableTileId)) {
                        ...
                        if (collidableTileId === CAR_TILE_ID_LEFT && !carSpawned && shouldSpawnCar) {
                            ...
                            const carPosition = PoolManager.getInstance().vector2.claim(x * getConfig().world.TILE_SIZE, y * getConfig().world.TILE_SIZE);
                        ...
                        boundary.setPosition(PoolManager.getInstance().vector2.claim(x * getConfig().world.TILE_SIZE, y * getConfig().world.TILE_SIZE));
    ...
    spawnDebugZombieIfEnabled() {
        if (DEBUG_START_ZOMBIE) {
            ...
            const poolManager = PoolManager.getInstance();
            zombie.setPosition(poolManager.vector2.claim(middleX + DEBUG_ZOMBIE_OFFSET_TILES * getConfig().world.TILE_SIZE, middleY));
    ...
    spawnIdleZombies() {
        ...
        for (let y = 0; y < totalSize; y++) {
            for (let x = 0; x < totalSize; x++) {
                ...
                const isValidGround = groundTile === GROUND_TILE_ID_1 ||
                ...
                if (isValidGround && !hasCollidable) {
                    if (this.mapRandom() < IDLE_ZOMBIE_SPAWN_CHANCE) {
                        const poolManager = PoolManager.getInstance();
                        const position = poolManager.vector2.claim(x * getConfig().world.TILE_SIZE, y * getConfig().world.TILE_SIZE);
    ...
    isOpenWorldMode() {
        return (this.getGameManagers().getGameServer().getGameLoop().getGameModeStrategy().getConfig()
            .modeId === "open_world");
    }
    /**
     * Collects map tiles eligible for open-world fixture zombies (same rules as spawnIdleZombies).
     */
    collectEligibleOpenWorldZombieTiles() {
        const totalSize = BIOME_SIZE * MAP_SIZE;
        const campsiteMinX = this.campsiteBiomeX * BIOME_SIZE;
        const campsiteMaxX = (this.campsiteBiomeX + 1) * BIOME_SIZE;
        const campsiteMinY = this.campsiteBiomeY * BIOME_SIZE;
    ...
    shuffleTileCoords(tiles) {
        for (let i = tiles.length - 1; i > 0; i--) {
            const j = Math.floor(this.mapRandom() * (i + 1));
            [tiles[i], tiles[j]] = [tiles[j], tiles[i]];
        }
    }
    pickTilesWithMinChebyshevSeparation(orderedCandidates, maxCount, minSep) {
        const picked = [];
        for (const t of orderedCandidates) {
            if (picked.length >= maxCount)
                break;
            const ok = picked.every((p) => Math.max(Math.abs(p.x - t.x), Math.abs(p.y - t.y)) >= minSep);
            if (ok)
                picked.push(t);
        }
        return picked;
    }
    seedOpenWorldZombieSpawnPoints() {
     ...
     */
    isAuthoredZombieFixtureSpawnValid(position, checkEntities = true, entitySize) {
        var _a;
        const { TILE_SIZE } = getConfig().world;
        const size = entitySize !== null && entitySize !== void 0 ? entitySize : TILE_SIZE;
        ...
        if (((_a = this.collidablesLayer[gridY]) === null || _a === void 0 ? void 0 : _a[gridX]) !== EMPTY_COLLIDABLE_TILE_ID) {
            return false;
        }
        if (this.doesPositionOverlapWithCar(position)) {
            return false;
        ...
        if (checkEntities) {
            const poolManager = PoolManager.getInstance();
            const positionCenter = poolManager.vector2.claim(position.x + size / 2, position.y + size / 2);
            ...
            for (const entity of nearbyEntities) {
                ...
                if (!entityBlocksPlacement(entityType))
                    continue;
    ...
    spawnSurvivorsInBiome(biomeX, biomeY) {
        if (!ENABLE_SURVIVOR_SPAWNS) {
            return;
        }
        // Spawn 1-2 survivors randomly
        const survivorCount = this.mapRandom() < SURVIVOR_SPAWN_PROBABILITY ? SURVIVOR_MIN_COUNT : SURVIVOR_MAX_COUNT;
    ...
    spawnSurvivorInRandomBiome() {
        ...
        if (validPositions.length === 0) {
            console.warn(`No valid positions to spawn survivor in biome at (${biomePosition.x}, ${biomePosition.y})`);
            return false;
        }
        const entity = this.getEntityManager().createEntity(Entities.SURVIVOR);
        if (!entity) {
    ...
    spawnBiomeItems(biome, biomeX, biomeY) {
    ...
    getRandomGrassPosition() {
        ...
        if (validPositions.length === 0) {
            ...
            const poolManager = PoolManager.getInstance();
            return poolManager.vector2.claim((totalSize * getConfig().world.TILE_SIZE) / 2, (totalSize * getConfig().world.TILE_SIZE) / 2);
        }
     ...
     */
    isPositionInSpecialBiome(position) {
        const TILE_SIZE = getConfig().world.TILE_SIZE;
        ...
        if (this.gasStationBiomePosition &&
            biomeX === this.gasStationBiomePosition.x &&
            biomeY === this.gasStationBiomePosition.y) {
            return true;
        }
        ...
        if (this.dockBiomePosition &&
            biomeX === this.dockBiomePosition.x &&
            biomeY === this.dockBiomePosition.y) {
            return true;
        }
        if (this.shedBiomePosition &&
            biomeX === this.shedBiomePosition.x &&
            biomeY === this.shedBiomePosition.y) {
            return true;
        }
     ...
     */
    doesPositionOverlapWithCar(position) {
    ...
    getRandomCampsitePosition() {
        ...
        for (let y = 0; y < BIOME_SIZE; y++) {
            for (let x = 0; x < BIOME_SIZE; x++) {
                ...
                if (isValidGround && this.collidablesLayer[mapY][mapX] === EMPTY_COLLIDABLE_TILE_ID) {
                    const poolManager = PoolManager.getInstance();
                    const position = poolManager.vector2.claim(mapX * getConfig().world.TILE_SIZE, mapY * getConfig().world.TILE_SIZE);
                    // Check if this position overlaps with the car
                    if (!this.doesPositionOverlapWithCar(position)) {
                        validPositions.push(position);
                    }
     ...
     */
    isPositionValidForPlacement(position, checkEntities = true, entitySize) {
        var _a, _b;
        const { TILE_SIZE } = getConfig().world;
        const size = entitySize !== null && entitySize !== void 0 ? entitySize : TILE_SIZE;
        const gridX = Math.floor(position.x / TILE_SIZE);
        ...
        const groundTile = (_a = this.groundLayer[gridY]) === null || _a === void 0 ? void 0 : _a[gridX];
        const isValidGround = groundTile === GROUND_TILE_ID_1 ||
            groundTile === GROUND_TILE_ID_2 ||
            groundTile === GROUND_TILE_ID_3 ||
            groundTile === GROUND_TILE_ID_4;
        ...
        if (((_b = this.collidablesLayer[gridY]) === null || _b === void 0 ? void 0 : _b[gridX]) !== EMPTY_COLLIDABLE_TILE_ID) {
            return false;
        }
     ...
     */
    getCampsiteSurroundingBiomeCenters() {
    ...
    getValidSpawnPositionsInBiome(biomeX, biomeY) {
        var _a, _b;
        const { TILE_SIZE } = getConfig().world;
        const validPositions = [];
        const poolManager = PoolManager.getInstance();
        ...
        for (let y = minTileY; y < maxTileY; y++) {
            for (let x = minTileX; x < maxTileX; x++) {
                // Check if it's a valid ground tile
                const groundTile = (_a = this.groundLayer[y]) === null || _a === void 0 ? void 0 : _a[x];
                const isValidGround = groundTile === GROUND_TILE_ID_1 ||
                    groundTile === GROUND_TILE_ID_2 ||
                    groundTile === GROUND_TILE_ID_3 ||
                    groundTile === GROUND_TILE_ID_4;
                ...
                // Check if there's a collidable
                if (((_b = this.collidablesLayer[y]) === null || _b === void 0 ? void 0 : _b[x]) !== EMPTY_COLLIDABLE_TILE_ID) {
                    continue;
                }
                // Convert tile coordinates to pixel coordinates
                const position = poolManager.vector2.claim(x * TILE_SIZE, y * TILE_SIZE);
                // Check if there are any zombies at this position
                const tileCenter = poolManager.vector2.claim(position.x + TILE_SIZE / 2, position.y + TILE_SIZE / 2);
                const nearbyEntities = this.getEntityManager().getNearbyEntities(tileCenter, TILE_SIZE / 2, zombieTypes);
     ...
     */
    findRandomValidSpawnPosition(center, minRadius, maxRadius) {
     ...
     */
    getEmptyGroundTiles(center, radius) {
        var _a, _b;
        const { TILE_SIZE } = getConfig().world;
        const totalSize = BIOME_SIZE * MAP_SIZE;
        ...
        for (let y = minTileY; y < maxTileY; y++) {
            for (let x = minTileX; x < maxTileX; x++) {
                ...
                if (center && radius !== undefined) {
                    const centerPos = poolManager.vector2.claim(position.x + TILE_SIZE / 2, position.y + TILE_SIZE / 2);
    ...
    spawnCrateInRandomBiome() {
        ...
        if (validPositions.length === 0) {
            console.warn(`No valid positions to spawn crate in biome at (${biomePosition.x}, ${biomePosition.y})`);
            return false;
        }
        // Pick a random position from valid positions
        const randomIndex = Math.floor(Math.random() * validPositions.length);
