...
export class Survivor extends Entity {
    constructor(gameManagers) {
        ...
        this.isWandering = false; // true = moving, false = paused
        this.campsiteCenter = null;
        this.initialSpawnPosition = null;
    ...
    updateWanderingAtCampsite(deltaTime) {
        var _a, _b, _c;
        ...
        if (!this.campsiteCenter) {
            ...
            if (campsitePos) {
                const campsite = (_a = this.getGameManagers().getMapManager().getMapData().biomePositions) === null || _a === void 0 ? void 0 : _a.campsite;
                ...
                this.campsiteCenter = poolManager.vector2.claim((centerBiomeX * BIOME_SIZE + BIOME_SIZE / 2) * TILE_SIZE, (centerBiomeY * BIOME_SIZE + BIOME_SIZE / 2) * TILE_SIZE);
            }
            else {
    ...
    updateWanderingAtSpawn(deltaTime) {
    ...
    updateWandering(deltaTime, wanderCenter) {
        ...
        if (distanceFromCenter > SURVIVOR_WANDER_RADIUS) {
            ...
            movable.setVelocity(poolManager.vector2.claim(direction.x * SURVIVOR_WANDER_SPEED, direction.y * SURVIVOR_WANDER_SPEED));
        ...
        if (this.isWandering) {
            ...
            if (this.wanderTimer >= WANDER_MOVE_DURATION) {
                ...
                const poolManager = PoolManager.getInstance();
                movable.setVelocity(poolManager.vector2.claim(this.wanderDirection.x * SURVIVOR_WANDER_SPEED, this.wanderDirection.y * SURVIVOR_WANDER_SPEED));
            }
        }
        else {
            // Currently paused
            if (this.wanderTimer >= WANDER_PAUSE_DURATION) {
                ...
                const testPos = poolManager.vector2.claim(currentPos.x + this.wanderDirection.x * SURVIVOR_WANDER_SPEED * WANDER_MOVE_DURATION, currentPos.y + this.wanderDirection.y * SURVIVOR_WANDER_SPEED * WANDER_MOVE_DURATION);
    ...
    handleMovement(deltaTime) {
        ...
        if (velocity.x !== 0) {
            const poolManager = PoolManager.getInstance();
            const attemptedPosition = poolManager.vector2.claim(originalPosition.x + velocity.x * deltaTime, originalPosition.y);
        ...
        if (velocity.y !== 0) {
            const poolManager = PoolManager.getInstance();
            const attemptedPosition = poolManager.vector2.claim(afterXPosition.x, afterXPosition.y + velocity.y * deltaTime);
            positionable.setPosition(attemptedPosition);
            if (this.getEntityManager().isColliding(this)) {
    ...
    tryShootAtZombie() {
        ...
        const nearbyEntities = this.getEntityManager().getNearbyEntities(this.getExt(Positionable).getPosition(), SURVIVOR_SHOOT_RANGE, zombieFilterSet);
    ...
    shootAt(target) {
        ...
        const poolManager = PoolManager.getInstance();
        const direction = poolManager.vector2.claim(targetPosition.x - survivorPosition.x, targetPosition.y - survivorPosition.y);
