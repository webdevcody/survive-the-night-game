...
export class WallLevel2 extends Entity {
  ...
  constructor(gameManagers: IGameManagers, itemState?: ItemState) {
    ...
    const count = itemState?.count ?? WallLevel2.DEFAULT_COUNT;
    const poolManager = PoolManager.getInstance();
    const size = poolManager.vector2.claim(16, 16);
    this.addExtension(new Positionable(this).setSize(size));
    this.addExtension(new Collidable(this).setSize(size));
    this.addExtension(
      new Interactive(this).onInteract(this.interact.bind(this)).setDisplayName("reinforced wall")
    );
    this.addExtension(
      new Destructible(this)
        .setMaxHealth(getConfig().world.WALL_LEVEL_2_MAX_HEALTH)
        .setHealth(itemState?.health ?? getConfig().world.WALL_LEVEL_2_MAX_HEALTH)
        .onDeath(() => this.onDeath())
    );
    this.addExtension(
      new Carryable(this, "wall_level_2").setItemState({
        count,
      })
    );
  ...
  private interact(entityId: number): void {
    ...
    stackableOptions.mergeStrategy = (existing, pickup) => {
      ...
      return {
        ...merged,
        health: pickup?.health ?? getConfig().world.WALL_LEVEL_2_MAX_HEALTH,
      };
    };
