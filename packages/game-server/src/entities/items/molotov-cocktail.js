...
export class MolotovCocktail extends Weapon {
    constructor(gameManagers, itemState) {
        ...
        if (this.hasExt(Carryable)) {
            ...
            const count = (_a = itemState === null || itemState === void 0 ? void 0 : itemState.count) !== null && _a !== void 0 ? _a : MolotovCocktail.DEFAULT_COUNT;
        ...
        if (this.hasExt(Interactive)) {
            ...
            this.interactiveExtension.onInteract((entityId) => {
                ...
                carryable.pickup(entityId, Carryable.createStackablePickupOptions(carryable, MolotovCocktail.DEFAULT_COUNT));
            });
        }
    }
    getCooldown() {
    ...
    attack(playerId, _position, facing, aimAngle, aimDistance) {
        ...
        const molotovIndex = inventoryItems.findIndex((item) => item && item.itemType === this.getType());
        if (molotovIndex === -1)
            return;
        ...
        const currentCount = ((_a = molotovItem.state) === null || _a === void 0 ? void 0 : _a.count) || 1;
        if (currentCount > 1) {
    ...
    explode() {
        ...
        const nearbyEntities = this.getEntityManager().getNearbyEntities(position, MolotovCocktail.EXPLOSION_RADIUS);
    ...
    spawnFires(centerPosition) {
        ...
        for (let i = 0; i < MolotovCocktail.FIRE_COUNT; i++) {
            ...
            const firePosition = poolManager.vector2.claim(centerPosition.x + Math.cos(angle) * distance, centerPosition.y + Math.sin(angle) * distance);
            // Create fire entity
...
MolotovCocktail.EXPLOSION_RADIUS = getConfig().combat.EXPLOSION_RADIUS_MEDIUM;
MolotovCocktail.EXPLOSION_DAMAGE = 3;
MolotovCocktail.THROW_SPEED = getConfig().combat.THROW_SPEED;
MolotovCocktail.DEFAULT_THROW_DISTANCE = getConfig().combat.TRAVEL_DISTANCE_MEDIUM;
MolotovCocktail.COOLDOWN = getConfig().combat.THROWABLE_COOLDOWN;
MolotovCocktail.DEFAULT_COUNT = 1;
MolotovCocktail.FIRE_COUNT = getConfig().combat.MOLOTOV_FIRE_COUNT;
MolotovCocktail.FIRE_SPREAD_RADIUS = getConfig().combat.MOLOTOV_FIRE_SPREAD_RADIUS;
