...
export class SentryGunLevel3 extends Entity {
  ...
  constructor(gameManagers: IGameManagers, itemState?: ItemState) {
    ...
    const count = itemState?.count ?? SentryGunLevel3.DEFAULT_COUNT;
    const poolManager = PoolManager.getInstance();
    const size = poolManager.vector2.claim(16, 16);
    this.addExtension(new Positionable(this).setSize(size));
    this.addExtension(new Collidable(this).setSize(size));
    this.addExtension(
      new Interactive(this).onInteract(this.interact.bind(this)).setDisplayName("advanced sentry gun")
    );
    this.addExtension(
      new Destructible(this)
        .setMaxHealth(getConfig().world.SENTRY_GUN_LEVEL_3_MAX_HEALTH)
        .setHealth(itemState?.health ?? getConfig().world.SENTRY_GUN_LEVEL_3_MAX_HEALTH)
        .onDeath(() => this.onDeath())
    );
    this.addExtension(
      new Carryable(this, "sentry_gun_level_3").setItemState({
        count,
      })
    );
  ...
  private interact(entityId: number): void {
    ...
    stackableOptions.mergeStrategy = (existing, pickup) => {
      ...
      return {
        ...merged,
        health: pickup?.health ?? getConfig().world.SENTRY_GUN_LEVEL_3_MAX_HEALTH,
      };
    };
