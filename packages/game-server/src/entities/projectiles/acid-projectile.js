...
export class AcidProjectile extends Entity {
    static get PROJECTILE_SIZE() {
        return PoolManager.getInstance().vector2.claim(8, 8);
    }
    constructor(gameManagers, startPosition, targetPosition) {
    ...
    update(deltaTime) {
        ...
        const destructibleEntities = this.getEntityManager().getNearbyIntersectingDestructableEntities(this);
...
AcidProjectile.PROJECTILE_SPEED = getConfig().combat.PROJECTILE_SPEED_SLOW;
AcidProjectile.PROJECTILE_DAMAGE = 1;
AcidProjectile.MAX_DISTANCE = getConfig().combat.TRAVEL_DISTANCE_MEDIUM;
