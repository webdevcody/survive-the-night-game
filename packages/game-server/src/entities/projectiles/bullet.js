...
export class Bullet extends Entity {
    constructor(gameManagers, damage = 1) {
    ...
    setDirection(direction) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(normalized.x * Bullet.BULLET_SPEED, normalized.y * Bullet.BULLET_SPEED));
    }
    setDirectionWithOffset(direction, offsetAngle) {
        ...
        const poolManager = PoolManager.getInstance();
        this.getExt(Movable).setVelocity(poolManager.vector2.claim((rotatedX / length) * Bullet.BULLET_SPEED, (rotatedY / length) * Bullet.BULLET_SPEED));
    }
    getHitbox() {
    ...
    setDirectionFromAngle(angle) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(dirX * Bullet.BULLET_SPEED, dirY * Bullet.BULLET_SPEED));
    }
    ...
    handleIntersections(fromPosition, toPosition) {
        ...
        const fromCenter = poolManager.vector2.claim(fromPosition.x + bulletCenterOffset, fromPosition.y + bulletCenterOffset);
        const toCenter = poolManager.vector2.claim(toPosition.x + bulletCenterOffset, toPosition.y + bulletCenterOffset);
        ...
        for (const target of targets) {
            ...
            const expandedPos = poolManager.vector2.claim(hitbox.position.x - bulletRadius, hitbox.position.y - bulletRadius);
            const expandedSize = poolManager.vector2.claim(hitbox.size.x + bulletRadius * 2, hitbox.size.y + bulletRadius * 2);
...
Bullet.BULLET_SPEED = getConfig().combat.BULLET_SPEED;
