...
export class FlameProjectile extends Entity {
    ...
    setDirection(direction) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(rotatedX * FlameProjectile.FLAME_SPEED, rotatedY * FlameProjectile.FLAME_SPEED));
    }
    /**
    ...
    setDirectionFromAngle(angle) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(dirX * FlameProjectile.FLAME_SPEED, dirY * FlameProjectile.FLAME_SPEED));
    }
    getHitbox() {
    ...
    handleIntersections(fromPosition, toPosition) {
        ...
        const fromCenter = poolManager.vector2.claim(fromPosition.x + flameRadius, fromPosition.y + flameRadius);
        const toCenter = poolManager.vector2.claim(toPosition.x + flameRadius, toPosition.y + flameRadius);
        ...
        for (const target of targets) {
            ...
            const expandedPos = poolManager.vector2.claim(hitbox.position.x - flameRadius, hitbox.position.y - flameRadius);
            const expandedSize = poolManager.vector2.claim(hitbox.size.x + flameRadius * 2, hitbox.size.y + flameRadius * 2);
...
FlameProjectile.FLAME_SPEED = getConfig().combat.PROJECTILE_SPEED_STANDARD;
