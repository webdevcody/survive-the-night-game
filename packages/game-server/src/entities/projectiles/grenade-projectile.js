...
import { calculateVelocityFromAngle, calculateVelocityFromDirection, } from "@/entities/weapons/helpers";
...
export class GrenadeProjectile extends Entity {
    constructor(gameManagers) {
        ...
        this.targetDistance = DEFAULT_TRAVEL_DISTANCE;
        this.shooterId = 0;
    ...
    setDirection(direction) {
        this.getExt(Movable).setVelocity(calculateVelocityFromDirection(direction, GRENADE_PROJECTILE_SPEED));
    }
    /**
    ...
    setDirectionFromAngle(angle) {
        this.getExt(Movable).setVelocity(calculateVelocityFromAngle(angle, GRENADE_PROJECTILE_SPEED));
    }
    setPosition(position) {
        this.getExt(Positionable).setPosition(position);
        this.startPosition = position;
    }
    updateGrenadeProjectile(deltaTime) {
        const poolManager = PoolManager.getInstance();
