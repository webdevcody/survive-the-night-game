...
export class ThrowingKnifeProjectile extends Entity {
    ...
    setDirection(direction) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(normalized.x * ThrowingKnifeProjectile.KNIFE_SPEED, normalized.y * ThrowingKnifeProjectile.KNIFE_SPEED));
    }
    setDirectionWithOffset(direction, offsetAngle) {
        ...
        const poolManager = PoolManager.getInstance();
        this.getExt(Movable).setVelocity(poolManager.vector2.claim((rotatedX / length) * ThrowingKnifeProjectile.KNIFE_SPEED, (rotatedY / length) * ThrowingKnifeProjectile.KNIFE_SPEED));
    }
    getHitbox() {
    ...
    setDirectionFromVelocity(velocity) {
        ...
        if (velocity.x === 0 && velocity.y === 0) {
            ...
            this.getExt(Movable).setVelocity(poolManager.vector2.claim(ThrowingKnifeProjectile.KNIFE_SPEED, 0));
            return;
        }
    ...
    setDirectionFromAngle(angle) {
        ...
        this.getExt(Movable).setVelocity(poolManager.vector2.claim(dirX * ThrowingKnifeProjectile.KNIFE_SPEED, dirY * ThrowingKnifeProjectile.KNIFE_SPEED));
    }
    updateKnife(deltaTime) {
    ...
    handleIntersections(fromPosition, toPosition) {
        ...
        for (const target of targets) {
            ...
            const expandedPos = poolManager.vector2.claim(hitbox.position.x - knifeRadius, hitbox.position.y - knifeRadius);
            const expandedSize = poolManager.vector2.claim(hitbox.size.x + knifeRadius * 2, hitbox.size.y + knifeRadius * 2);
...
ThrowingKnifeProjectile.KNIFE_SPEED = getConfig().combat.PROJECTILE_SPEED_STANDARD;
