...
export class SplitterBoss extends BossEnemy {
    constructor(gameManagers, splitGeneration = 0, splitsRemaining = 3) {
    ...
    performSplit() {
        ...
        for (let i = 0; i < 2; i++) {
            ...
            while (!spawnPosition && attempts < maxAttempts) {
                const candidatePosition = mapManager.findRandomValidSpawnPosition(center, getConfig().boss.SPLITTER_BOSS_MIN_SPLIT_RADIUS, getConfig().boss.SPLITTER_BOSS_MAX_SPLIT_RADIUS);
                if (candidatePosition) {
                    // Check if this position is far enough from already used positions
                    let isValidPosition = true;
                    const poolManager = PoolManager.getInstance();
                    const candidateCenter = poolManager.vector2.claim(candidatePosition.x + 16, candidatePosition.y + 16);
            ...
            if (!spawnPosition) {
                ...
                const offset = PoolManager.getInstance().vector2.claim(Math.cos(angle) * fallbackDistance, Math.sin(angle) * fallbackDistance);
                spawnPosition = PoolManager.getInstance().vector2.claim(center.x + offset.x, center.y + offset.y);
            ...
            newBoss.lastHealth = newDestructible.getMaxHealth();
        ...
        if (newIds.length > 0) {
            this.getGameManagers()
                .getBroadcaster()
                .broadcastEvent(new BossSplitEvent({
