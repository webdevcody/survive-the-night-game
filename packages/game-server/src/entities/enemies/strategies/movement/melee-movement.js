...
export class MeleeMovementStrategy {
    constructor() {
        this.pathRecalculationTimer = Math.random() * MeleeMovementStrategy.PATH_RECALCULATION_INTERVAL;
        this.targetChecker = new TargetChecker();
        this.currentWaypoint = null;
        this.lastWaypointPosition = null; // Track position when waypoint was set
        this.stuckTimer = 0; // Track time spent trying to reach current waypoint
    }
    update(zombie, deltaTime) {
        ...
        const needNewWaypoint = !this.currentWaypoint ||
            reachedWaypoint ||
...
MeleeMovementStrategy.PATH_RECALCULATION_INTERVAL = getConfig().entity.PATH_RECALCULATION_INTERVAL;
MeleeMovementStrategy.STUCK_DETECTION_TIME = getConfig().entity.STUCK_DETECTION_TIME;
MeleeMovementStrategy.WAYPOINT_REACHED_THRESHOLD = 8; // Increased threshold for reaching waypoint
