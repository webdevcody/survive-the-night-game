...
export class RangedMovementStrategy {
    constructor() {
        this.pathRecalculationTimer = Math.random() * RangedMovementStrategy.PATH_RECALCULATION_INTERVAL;
    ...
    update(zombie, deltaTime) {
        ...
        if (this.currentWaypoint) {
            ...
            const pathfindingVelScaled = poolManager.vector2.claim(pathfindingVelocity.x * speed, pathfindingVelocity.y * speed);
            // Apply separation force to avoid clustering
            const separationForce = calculateSeparationForce(zombie);
            const finalVelocity = blendSeparationForce(pathfindingVelScaled, separationForce);
            zombie.getExt(Movable).setVelocity(finalVelocity);
...
RangedMovementStrategy.ATTACK_RANGE = getConfig().combat.RANGED_ATTACK_RANGE;
RangedMovementStrategy.PATH_RECALCULATION_INTERVAL = getConfig().entity.PATH_RECALCULATION_INTERVAL;
