...
export class ChargingMovementStrategy {
    constructor(chargeSpeedMultiplier = 3) {
    ...
    update(zombie, deltaTime) {
        ...
        const direction = normalizeVector(PoolManager.getInstance().vector2.claim(target.position.x - zombiePos.x, target.position.y - zombiePos.y));
        // Move at charge speed directly toward player
        const chargeSpeed = zombie.getSpeed() * this.chargeSpeedMultiplier;
        const poolManager = PoolManager.getInstance();
        const velocity = poolManager.vector2.claim(direction.x * chargeSpeed, direction.y * chargeSpeed);
