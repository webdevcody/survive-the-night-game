 ...
 */
export function calculateSeparationForce(zombie) {
    ...
    for (const otherZombie of nearbyZombies) {
        ...
        if (dist < entityConfig.ZOMBIE_MIN_SEPARATION_DISTANCE ||
    ...
    if (totalWeight > 0) {
        ...
        if (magnitude > 0) {
            const normalizedSeparation = normalizeVector(poolManager.vector2.claim(separationX, separationY));
            const separationForce = poolManager.vector2.claim(normalizedSeparation.x * entityConfig.ZOMBIE_SEPARATION_STRENGTH, normalizedSeparation.y * entityConfig.ZOMBIE_SEPARATION_STRENGTH);
 ...
 */
export function blendSeparationForce(pathfindingVelocity, separationForce) {
    const poolManager = PoolManager.getInstance();
    const entityConfig = getConfig().entity;
    // Blend: (1 - weight) * pathfinding + weight * separation
    const pathWeight = 1 - entityConfig.ZOMBIE_SEPARATION_WEIGHT;
    const blendedX = pathfindingVelocity.x * pathWeight + separationForce.x * entityConfig.ZOMBIE_SEPARATION_WEIGHT;
    const blendedY = pathfindingVelocity.y * pathWeight + separationForce.y * entityConfig.ZOMBIE_SEPARATION_WEIGHT;
