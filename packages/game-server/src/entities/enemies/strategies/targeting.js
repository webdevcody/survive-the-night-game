...
export class TargetingSystem {
     ...
     */
    static findClosestFriendlyEntity(zombie, searchRadius = 500, filterTypes) {
        const zombiePos = zombie.getCenterPosition();
        // Use a broad filter for performance, then filter by group
        const typesSet = filterTypes ? new Set(filterTypes) : FRIENDLY_TYPES;
     ...
     */
    static findNearbyAttackableEntities(zombie, searchRadius) {
        const zombiePos = zombie.getCenterPosition();
        const nearbyEntities = zombie
            .getEntityManager()
     ...
     */
    static findClosestAttackableEntity(zombie, searchRadius) {
     ...
     */
    static findClosestPlayer(zombie, searchRadius) {
