...
export class GroundSlamAttackStrategy {
    constructor(slamRadius = 64, slamDamage = 1, knockbackForce = 400) {
    ...
    performGroundSlam(zombie) {
        ...
        for (const entity of nearbyEntities) {
            if (!entity.hasExt(Destructible) || !entity.hasExt(Positionable))
                continue;
            ...
            if (dist <= this.slamRadius) {
                ...
                if (entity instanceof Player && entity.hasExt(Movable)) {
                    const knockbackDirection = normalizeVector(PoolManager.getInstance().vector2.claim(entityPos.x - position.x, entityPos.y - position.y));
                    const poolManager = PoolManager.getInstance();
                    const knockbackVelocity = poolManager.vector2.claim(knockbackDirection.x * this.knockbackForce, knockbackDirection.y * this.knockbackForce);
        ...
        zombie.getEntityManager().getBroadcaster().broadcastEvent(new ExplosionEvent({
            position,
        }));
    }
}
