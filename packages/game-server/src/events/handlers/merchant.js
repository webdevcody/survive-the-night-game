 ...
 */
function validateMerchantBuyData(data) {
 ...
 */
function validateMerchantSellData(data) {
    if (typeof data !== "object" || data === null) {
        return null;
    }
    const obj = data;
 ...
 */
function getDefaultCountForItem(itemType, gameManagers) {
    ...
    const constructorWithStatic = entityConstructor;
    ...
    try {
        ...
        if (tempInstance instanceof StackableItem) {
            return StackableItem.getDefaultCount(entityConstructor, gameManagers);
        }
    }
    catch (_a) {
...
export function onMerchantBuy(context, socket, data) {
    var _a, _b, _c, _d;
    ...
    const item = defaultCount !== undefined ? { itemType, state: { count: defaultCount } } : { itemType };
    const items = inventory.getItems();
    const isStackable = isStackableItem(item);
    let merged = false;
    if (isStackable && ((_a = item.state) === null || _a === void 0 ? void 0 : _a.count)) {
        const existingItemIndex = items.findIndex((it) => it != null && it.itemType === itemType);
        if (existingItemIndex >= 0) {
            const existingItem = items[existingItemIndex];
            if (existingItem) {
                const existingCount = (_d = (_c = (_b = existingItem.state) === null || _b === void 0 ? void 0 : _b.count) !== null && _c !== void 0 ? _c : defaultCount) !== null && _d !== void 0 ? _d : 1;
                const newCount = existingCount + item.state.count;
                inventory.updateItemState(existingItemIndex, { count: newCount });
                merged = true;
                context.broadcastEvent(new PlayerPickedUpItemEvent({
...
export function onMerchantSell(context, socket, data) {
    var _a, _b, _c, _d, _e;
    const player = context.players.get(socket.id);
    if (!player)
        return;
    ...
    if ((_a = weaponConfig === null || weaponConfig === void 0 ? void 0 : weaponConfig.merchant) === null || _a === void 0 ? void 0 : _a.price) {
        buyPrice = weaponConfig.merchant.price;
    }
    else {
        const itemConfig = itemRegistry.get(itemType);
        if ((_b = itemConfig === null || itemConfig === void 0 ? void 0 : itemConfig.merchant) === null || _b === void 0 ? void 0 : _b.price) {
            buyPrice = itemConfig.merchant.price;
        }
        else {
            const resourceConfig = resourceRegistry.get(itemType);
            if ((_c = resourceConfig === null || resourceConfig === void 0 ? void 0 : resourceConfig.merchant) === null || _c === void 0 ? void 0 : _c.price) {
    ...
    const currentCount = (_e = (_d = item.state) === null || _d === void 0 ? void 0 : _d.count) !== null && _e !== void 0 ? _e : (isStackable ? (defaultCount !== null && defaultCount !== void 0 ? defaultCount : 1) : 1);
...
export const merchantBuyHandler = {
    event: "MERCHANT_BUY",
...
export const merchantSellHandler = {
    event: "MERCHANT_SELL",
    handler: (context, socket, data) => {
