...
import { canItemGoInEquipmentSlot, decodeEquipmentSlotKey, } from "@shared/util/inventory";
...
function isStackableItem(item) {
    ...
    return (itemConfig === null || itemConfig === void 0 ? void 0 : itemConfig.category) === "ammo";
}
function validateBankPayload(data) {
    ...
    if (typeof lockerEntityId !== "number" ||
...
function assertLockerInRange(player, lockerEntityId) {
    ...
    return (distance(player.getCenterPosition(), stationEntity.getExt(Positionable).getCenterPosition()) <= getConfig().player.MAX_INTERACT_RADIUS);
}
function firstEmptyBagIndex(inv) {
...
function firstEmptyBankIndex(bank) {
...
export function dropItemNearPlayerFacing(player, itemToDrop) {
    var _a;
    ...
    function tryCombineAtPosition(x, y, radius) {
        if (!isStackable) {
            return false;
        }
        ...
        for (let i = 0; i < nearby.length; i++) {
            ...
            if (distance(checkPos, otherPos) <= radius) {
                ...
                entityManager.getBroadcaster().broadcastEvent(new PlayerDroppedItemEvent({
...
function bankStashFromBag(player, bank, inv, bagIndex) {
...
function bankStashFromEquipment(player, bank, inv, equipSlot) {
...
function bankWithdrawToBag(player, bank, inv, bankIndex) {
...
function bankDropFromSource(player, bank, inv, source, slotIndex) {
    if (source === 0) {
...
function bankUseFromSource(player, bank, inv, source, slotIndex) {
    let item;
    let restoreBankIdx = null;
    if (source === 1) {
...
function bankEquipFromSource(player, bank, inv, source, slotIndex, equipSlotIndex) {
    let item;
    let restoreBankIdx = null;
    let restoreBagIdx = null;
    if (source === 1) {
        item = bank.removeItem(slotIndex);
        restoreBankIdx = slotIndex;
    }
    ...
    if (loadoutRow !== null) {
        if (loadoutRow <= 2) {
            if (!itemMatchesLoadoutRow(item.itemType, loadoutRow)) {
                inv.removeItem(empty);
                if (restoreBankIdx !== null) {
                    bank.setBankSlot(restoreBankIdx, item);
                }
...
export function onBankAction(context, socket, data) {
...
export const bankActionHandler = {
    event: "BANK_ACTION",
