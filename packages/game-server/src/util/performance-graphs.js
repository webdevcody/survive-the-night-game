 ...
 */
export function renderMethodBarGraph(methodStats) {
    ...
    const renderTree = (stats, prefix = "", isLast = []) => {
        for (let i = 0; i < stats.length; i++) {
            ...
            // Render this node
            console.log(`   ${currentPrefix}${stat.name.padEnd(20)} │${colorCode}${bar}${padding}\x1b[0m │ ${stat.percentage.toFixed(1)}% (${stat.averageTime.toFixed(2)}ms avg)`);
 ...
 */
export function renderTickTimeGraph(history, currentStats) {
    if (history.length === 0) {
        console.log("   (no data)");
        return;
    }
    ...
    console.log(`   Legend: \x1b[32m▁\x1b[0m=Min (${currentStats.min.toFixed(2)}ms) │ \x1b[33m●\x1b[0m=Avg (${currentStats.avg.toFixed(2)}ms) │ \x1b[31m▇\x1b[0m=Max (${currentStats.max.toFixed(2)}ms)`);
}
/**
 * Renders a line graph showing bandwidth history with min/avg/max indicators
 * Similar to tick time graph but for bandwidth
 */
export function renderBandwidthGraph(history, stats) {
    if (history.length === 0) {
        console.log("   (no data)");
        return;
    }
    ...
    // Print legend with colors
    console.log(`   Legend: \x1b[32m▁\x1b[0m=Min (${formatBytes(stats.min)}) │ \x1b[33m●\x1b[0m=Avg (${formatBytes(stats.avg)}) │ \x1b[31m▇\x1b[0m=Max (${formatBytes(stats.max)})`);
}
/**
