...
export class EnvironmentalEventManager {
  ...
  private activeToxicGasEvent: boolean = false;
   ...
   */
  private startToxicGasEvent(): void {
    this.activeToxicGasEvent = true;
    this.toxicGasClouds = [];
  ...
  /**
   * End toxic gas event - stop all clouds from reproducing and remove them
   */
  private endToxicGasEvent(): void {
    this.activeToxicGasEvent = false;
  ...
  /**
   * Check if toxic gas event is active
   */
  public isToxicGasEventActive(): boolean {
    return this.activeToxicGasEvent;
  }
}
