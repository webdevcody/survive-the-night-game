...
export class EventRateLimiter {
    ...
    checkEvent(socketId, eventType) {
        ...
        if (timestamps.length >= limit.maxEvents) {
            ...
            console.warn(`Socket ${socketId} exceeded rate limit for ${eventType} (${timestamps.length}/${limit.maxEvents} in ${limit.windowMs}ms). Strike ${currentStrikes}/${this.STRIKES_BEFORE_BAN}`);
            // Check if should ban
            if (currentStrikes >= this.STRIKES_BEFORE_BAN) {
                this.bannedSockets.set(socketId, now + this.BAN_DURATION_MS);
                console.warn(`Socket ${socketId} banned for ${this.BAN_DURATION_MS / 1000}s due to repeated rate limit violations`);
                return {
                    allowed: false,
                    shouldBan: true,
