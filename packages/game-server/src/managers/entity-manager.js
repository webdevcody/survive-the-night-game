...
import { createEntityWithFactory } from "@shared/util/entity-factory-pattern";
...
const STATIC_ENTITIES = [Entities.BOUNDARY, Entities.CAR];
export class EntityManager {
    constructor() {
        this.entityMap = new Map(); // Fast lookup by ID
        this.entitiesByType = new Map(); // Fast lookup by type
        this.zombies = [];
        this.merchants = [];
        this.entitiesToRemove = [];
        this.availableIds = [];
        this.maxId = 65535; // Maximum ID value (uint16 max)
        this.nextNewId = 0; // Counter for generating new IDs when pool is empty
        this.entityFinder = null;
        this.dynamicEntities = [];
        this.updatableEntities = []; // Entities that have extensions with update methods
        this.dirtyEntities = new Set();
        this.entitiesInGrid = new Set();
        this.entitiesToAddToGrid = new Set();
    ...
    getEntitiesByType(type) {
        var _a;
        return (_a = this.entitiesByType.get(type)) !== null && _a !== void 0 ? _a : [];
    }
    hasRegisteredItem(type) {
    ...
    createEntityFromItem(item) {
        var _a;
        const entityType = item.itemType;
        ...
        if (((_a = item.state) === null || _a === void 0 ? void 0 : _a.health) !== undefined && entity.hasExt(Destructible)) {
    ...
    addEntity(entity) {
        ...
        if (this.entityMap.has(entity.getId())) {
            console.error(`[Server] CRITICAL: addEntity called for existing ID ${entity.getId()}. Force cleaning up old entity to prevent corruption.`);
        ...
        if (entity.hasExt(Positionable)) {
            ...
            positionable.setOnPositionChange((changedEntity) => {
                this.dirtyEntities.add(changedEntity);
            });
    ...
    markEntityForRemoval(entity, expiration = 0) {
     ...
     */
    cleanupEntityFromTracking(entity) {
    ...
    generateEntityId() {
        ...
        if (activeEntityCount >= this.maxId + 1) {
            ...
            throw new Error(`Entity ID pool exhausted. Max ID: ${this.maxId}, Active entities: ${activeEntityCount}. ` +
        ...
        console.error(`CRITICAL: Entity ID pool empty but only ${activeEntityCount} entities exist ` +
        ...
        throw new Error(`Entity ID pool exhausted and recovery failed. Max ID: ${this.maxId}, ` +
            `Active entities: ${activeEntityCount}, Available IDs: ${this.availableIds.length}`);
    }
    isEntityMarkedForRemoval(entityId) {
    ...
    pruneEntities() {
        ...
        const entitiesToRemoveMap = new Map([]);
    ...
    getNearbyEntities(position, radius = 64, filterSet) {
        var _a, _b;
        ...
        const entities = (_b = (_a = this.entityFinder) === null || _a === void 0 ? void 0 : _a.getNearbyEntities(position, radius, filterSet)) !== null && _b !== void 0 ? _b : [];
    ...
    getClosestPlayer(entity) {
        ...
        let closestPlayerDistance = distance(entityPosition, this.players[closestPlayerIdx].getPosition());
    ...
    getClosestAlivePlayer(entity, attackRange) {
        if (!entity.hasExt(Positionable)) {
            return null;
        }
        if (this.players.length === 0) {
        ...
        for (const player of this.players) {
            ...
            if (attackRange && playerDistance > attackRange)
                continue;
            if (playerDistance < closestPlayerDistance) {
    ...
    getNearbyIntersectingDestructableEntities(sourceEntity) {
     ...
     */
    getIntersectingCollidableEntity(sourceEntity, ignoreTypes) {
        if (!this.entityFinder) {
            return null;
        }
    ...
    isColliding(sourceEntity, ignoreTypes) {
        return this.getIntersectingCollidableEntity(sourceEntity, ignoreTypes);
    }
    update(deltaTime) {
        var _a, _b;
        // Track refreshSpatialGrid as sub-method
        const endRefreshSpatialGrid = ((_a = this.tickPerformanceTracker) === null || _a === void 0 ? void 0 : _a.startMethod("refreshSpatialGrid", "updateEntities")) ||
            (() => { });
        ...
        const endUpdateExtensionsLoop = ((_b = this.tickPerformanceTracker) === null || _b === void 0 ? void 0 : _b.startMethod("updateExtensionsLoop", "updateEntities")) ||
            (() => { });
    ...
    updateExtensions(entity, deltaTime) {
        var _a, _b;
        // Track entity update time if performance monitoring is enabled
        const endEntityUpdate = ((_a = this.tickPerformanceTracker) === null || _a === void 0 ? void 0 : _a.startEntityUpdate(entity.getId(), entity.getType())) ||
            (() => { });
        // Track extension updates
        const endExtensionUpdates = ((_b = this.tickPerformanceTracker) === null || _b === void 0 ? void 0 : _b.startMethod("extensionUpdates", "updateExtensionsLoop")) ||
            (() => { });
        for (const extension of entity.getUpdatableExtensions()) {
            extension.update(deltaTime);
        }
        endExtensionUpdates();
        endEntityUpdate();
    }
    refreshSpatialGrid() {
        ...
        if (this.entitiesInGrid.size === 0) {
            const endInitialPopulation = ((_a = this.tickPerformanceTracker) === null || _a === void 0 ? void 0 : _a.startMethod("initialGridPopulation", "refreshSpatialGrid")) ||
                (() => { });
        ...
        const endDirtyEntityUpdates = ((_b = this.tickPerformanceTracker) === null || _b === void 0 ? void 0 : _b.startMethod("dirtyEntityUpdates", "refreshSpatialGrid")) ||
            (() => { });
        ...
        const endNewEntityAdditions = ((_c = this.tickPerformanceTracker) === null || _c === void 0 ? void 0 : _c.startMethod("newEntityAdditions", "refreshSpatialGrid")) ||
            (() => { });
