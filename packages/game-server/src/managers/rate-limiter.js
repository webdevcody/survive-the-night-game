...
export class RateLimiter {
    constructor() {
        ...
        this.bans = new Map();
        // Configuration
        this.CONNECTION_WINDOW_MS = 60 * 1000; // 1 minute
        this.MAX_CONNECTIONS_PER_WINDOW = 10; // More than 1 connection in 1 minute triggers ban
     ...
     */
    recordConnectionAttempt(displayName) {
        const now = Date.now();
        ...
        if (attempts.length > this.MAX_CONNECTIONS_PER_WINDOW) {
            ...
            console.warn(`User ${displayName} exceeded connection limit (${attempts.length} connections in past minute). Banned for 1 hour.`);
            return true;
        }
        return false;
    }
