...
import { ThreatScorer } from "./ai-threat-scorer";
/**
...
export class AITargetingSystem {
    ...
    getThreatInfo(player) {
        ...
        // Check for zombies
        const nearbyEntities = entityManager.getNearbyEntities(playerPos, AI_CONFIG.ZOMBIE_DETECTION_RADIUS);
     ...
     */
    getEnhancedThreatInfo(player, damageHistory, currentWeaponType) {
        var _a, _b, _c;
        const playerPos = player.getCenterPosition();
        const entityManager = this.gameManagers.getEntityManager();
        const zombieTypes = getZombieTypesSet();
        ...
        const weaponRange = currentWeaponType
            ? (_a = WEAPON_RANGES[currentWeaponType]) !== null && _a !== void 0 ? _a : AI_CONFIG.MELEE_RANGE
        ...
        // Scan zombies
        const nearbyEntities = entityManager.getNearbyEntities(playerPos, Math.max(AI_CONFIG.ZOMBIE_DETECTION_RADIUS, AI_CONFIG.PLAYER_DETECTION_RADIUS));
        for (const entity of nearbyEntities) {
            const entityType = entity.getType();
            ...
            if (zombieTypes.has(entityType)) {
                ...
                const assessment = ThreatScorer.assessThreat(playerPos, entity, damageHistory, weaponRange);
                if (assessment) {
                    ...
                    if (entity.hasExt(Positionable)) {
                        ...
                        const dir = poolManager.vector2.claim(enemyPos.x - playerPos.x, enemyPos.y - playerPos.y);
        ...
        for (const otherPlayer of players) {
            ...
            const assessment = ThreatScorer.assessThreat(playerPos, otherPlayer, damageHistory, weaponRange);
            if (assessment) {
                threats.push(assessment);
                playerCount++;
                // Track direction for surrounded detection
        ...
        const safestRetreatDirection = this.calculateSafestRetreatDirection(playerPos, threatDirections);
        // Release pooled direction vectors
        for (const dir of threatDirections) {
            poolManager.vector2.release(dir);
        }
        // Backward compatible fields
        const hasImmediateThreat = immediateThreat !== null &&
            immediateThreat.distance <= AI_CONFIG.IMMEDIATE_THREAT_RADIUS;
        const hasNearbyEnemy = immediateThreat !== null &&
            immediateThreat.distance <= AI_CONFIG.COMBAT_ENGAGE_RADIUS;
        const nearestEnemyDistance = (_b = immediateThreat === null || immediateThreat === void 0 ? void 0 : immediateThreat.distance) !== null && _b !== void 0 ? _b : Infinity;
        const enemyType = (_c = immediateThreat === null || immediateThreat === void 0 ? void 0 : immediateThreat.entityType) !== null && _c !== void 0 ? _c : "none";
        return {
            immediateThreat,
     ...
     */
    checkIfSurrounded(directions) {
        if (directions.length < AI_CONFIG.SURROUNDED_THRESHOLD)
            return false;
     ...
     */
    calculateSafestRetreatDirection(playerPos, threatDirections) {
     ...
     */
    findBestEnemy(player, damageHistory, currentWeaponType) {
        const enhancedInfo = this.getEnhancedThreatInfo(player, damageHistory, currentWeaponType);
        if (!enhancedInfo.immediateThreat)
            return null;
        ...
        return {
            ...
            priority: threat.distance <= AI_CONFIG.IMMEDIATE_THREAT_RADIUS
    ...
    findBestLootTarget(player) {
        ...
        const ownedWeaponTypes = new Set(ownedWeapons);
        const usefulAmmoTypes = new Set();
        ...
        for (const entity of nearbyEntities) {
            ...
            if (!entity.hasExt(Carryable) || !entity.hasExt(Positionable))
                continue;
            ...
            if (ALL_WEAPONS.includes(itemType) && ownedWeaponTypes.has(itemType)) {
                continue;
            }
    ...
    findBestPlayerTarget(player) {
        ...
        for (const otherPlayer of players) {
            ...
            const otherPos = otherPlayer.getCenterPosition();
            if (this.pathfinder.isToxicPosition(otherPos))
                continue;
            const dist = distance(playerPos, otherPos);
     ...
     */
    findSafeRetreatPosition(player) {
        const playerPos = player.getCenterPosition();
        const entityManager = this.gameManagers.getEntityManager();
        const zombieTypes = getZombieTypesSet();
        ...
        const players = entityManager.getPlayerEntities();
        for (const otherPlayer of players) {
            if (otherPlayer.getId() === player.getId() || otherPlayer.isDead())
                continue;
            const enemyPos = otherPlayer.getCenterPosition();
        ...
        const poolManager = PoolManager.getInstance();
        const totalSize = getConfig().world.MAP_SIZE * getConfig().world.BIOME_SIZE * getConfig().world.TILE_SIZE;
     ...
     */
    findNearestSpecialBiome(player, exploredBiomes) {
        ...
        for (const biome of specialBiomes) {
            ...
            if (exploredBiomes.has(biomeKey))
                continue;
            ...
            if (this.pathfinder.isToxicPosition(biomeCenter))
                continue;
     ...
     */
    getExploreTarget(player, exploredCells) {
        const playerPos = player.getCenterPosition();
        const entityManager = this.gameManagers.getEntityManager();
        const TILE_SIZE = getConfig().world.TILE_SIZE;
     ...
     */
    findOpportunisticPickup(player) {
        const playerPos = player.getCenterPosition();
        const inventory = player.getInventory();
        const entityManager = this.gameManagers.getEntityManager();
        const nearbyEntities = entityManager.getNearbyEntities(playerPos, AI_CONFIG.OPPORTUNISTIC_PICKUP_RADIUS);
        let bestTarget = null;
        let bestScore = -1;
        ...
        for (const entity of nearbyEntities) {
            ...
            if (GOOD_WEAPONS.includes(itemType)) {
     ...
     */
    findOpportunisticCrate(player) {
        const playerPos = player.getCenterPosition();
        const entityManager = this.gameManagers.getEntityManager();
        const nearbyEntities = entityManager.getNearbyEntities(playerPos, AI_CONFIG.OPPORTUNISTIC_CRATE_RADIUS);
    ...
    hasGoodWeapon(inventory) {
        return inventory.some((item) => item && GOOD_WEAPONS.includes(item.itemType));
    }
    needsAmmo(inventory) {
        var _a, _b;
        const weapon = inventory.find((item) => item && Object.keys(WEAPON_AMMO_MAP).includes(item.itemType));
        if (!weapon)
            return false;
        ...
        return !ammo || ((_b = (_a = ammo.state) === null || _a === void 0 ? void 0 : _a.count) !== null && _b !== void 0 ? _b : 0) < 10;
    }
    /**
     ...
     */
    canPickUpItem(inventory, itemType) {
     ...
     */
    getOwnedWeapons(inventory) {
        return inventory
            .filter((item) => item && ALL_WEAPONS.includes(item.itemType))
     ...
     */
    findUselessAmmoIndex(inventory) {
        const ownedWeapons = this.getOwnedWeapons(inventory);
        // Build a set of ammo types we need
        const neededAmmoTypes = new Set();
        for (const weapon of ownedWeapons) {
            const ammoType = WEAPON_AMMO_MAP[weapon];
            if (ammoType) {
    ...
    getLootPriority(itemType, needsWeapon, needsAmmo, needsHealth) {
        ...
        if (needsWeapon && GOOD_WEAPONS.includes(itemType)) {
