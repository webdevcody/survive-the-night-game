...
export class AIPlayerManager {
     ...
     */
    spawnAIPlayers(count = AI_CONFIG.DEFAULT_AI_COUNT) {
     ...
     */
    hookDamageEvents(player, controller) {
        ...
        destructible.onDamaged((attackerId, damage) => {
            if (attackerId === undefined || damage === undefined)
                return;
            ...
            const isZombie = zombieTypes.has(attackerType);
            const entityType = isZombie ? "zombie" : "player";
     ...
     */
    adjustAIPlayerCount(realPlayerCount) {
        const config = getConfig().aiPlayer;
        const targetAI = Math.max(config.MIN_AI_PLAYERS, config.TOTAL_PLAYER_THRESHOLD - realPlayerCount);
