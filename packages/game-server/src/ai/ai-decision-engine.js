 ...
 */
export var AIDecision;
(function (AIDecision) {
    AIDecision["ESCAPE"] = "ESCAPE";
    AIDecision["ENGAGE_ATTACKER"] = "ENGAGE_ATTACKER";
    AIDecision["ENGAGE_THREAT"] = "ENGAGE_THREAT";
    AIDecision["FINISH_KILL"] = "FINISH_KILL";
    AIDecision["RETREAT_AND_FIGHT"] = "RETREAT_AND_FIGHT";
    AIDecision["RETREAT"] = "RETREAT";
    AIDecision["DISENGAGE"] = "DISENGAGE";
    AIDecision["HUNT"] = "HUNT";
    AIDecision["LOOT"] = "LOOT";
    AIDecision["EXPLORE"] = "EXPLORE";
...
export class AIDecisionEngine {
     ...
     */
    static makeEnhancedDecision(player, threatInfo, supplyStatus, currentState, stateTimer, isAllInMode, hasLootTarget) {
     ...
     */
    static makeDecision(player, threatInfo, hasHealing, hasWeapon, hasLootTarget) {
        const healthPercent = player.getHealth() / player.getMaxHealth();
        ...
        // ===========================
        if (threatInfo.enemyCount >= AI_CONFIG.OUTNUMBERED_RETREAT_THRESHOLD &&
        ...
        if (threatInfo.hasImmediateThreat || threatInfo.hasNearbyEnemy) {
            ...
            const shouldRetreatFromThreat = (healthPercent < AI_CONFIG.RETREAT_HEALTH_THRESHOLD && hasHealing) ||
                !hasWeapon ||
     ...
     */
    static findCurrentAttacker(threatInfo) {
     ...
     */
    static findFinishableEnemy(threatInfo, myHealthPercent) {
        // Don't try to finish kills if we're too hurt
        if (myHealthPercent < AI_CONFIG.FINISH_KILL_MY_HP) {
            return null;
        }
        for (const threat of threatInfo.threats) {
            if (threat.healthPercent < AI_CONFIG.FINISH_KILL_ENEMY_HP &&
     ...
     */
    static shouldForceRetarget(currentTargetId, attackerId) {
     ...
     */
    static getRetargetInterval(isInCombat) {
        return isInCombat
