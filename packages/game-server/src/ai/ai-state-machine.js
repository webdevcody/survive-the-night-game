 ...
 */
export var KitePhase;
(function (KitePhase) {
    KitePhase["APPROACH"] = "APPROACH";
    KitePhase["ATTACK"] = "ATTACK";
    KitePhase["RETREAT"] = "RETREAT";
    KitePhase["DISENGAGE"] = "DISENGAGE";
})(KitePhase || (KitePhase = {}));
/**
...
export class AIStateMachine {
    constructor() {
        this.currentState = AIState.LOOT;
        ...
        this.retreatCommitted = false; // True when we've entered retreat
        this.allInMode = false; // True when retreat timer expired without healing
    }
    getCurrentState() {
     ...
     */
    update(player, threatInfo, hasLootTarget, deltaTime) {
     ...
     */
    updateWithDecisionEngine(player, enhancedThreatInfo, hasLootTarget, deltaTime) {
        this.stateTimer += deltaTime;
        const inventory = player.getInventory();
        const hasHealing = this.hasBandage(inventory);
        const hasWeapon = this.hasAnyWeapon(inventory);
        // Use the decision engine to determine what to do
        const decision = AIDecisionEngine.makeDecision(player, enhancedThreatInfo, hasHealing, hasWeapon, hasLootTarget);
     ...
     */
    canExitCurrentState(targetState) {
     ...
     */
    canInterrupt(fromState, toState) {
    ...
    hasAmmoForWeapons(inventory) {
        var _a, _b;
        const weapons = inventory.filter((item) => item && Object.keys(WEAPON_AMMO_MAP).includes(item.itemType));
        if (weapons.length === 0) {
            // No ranged weapons, check for melee
            return inventory.some((item) => item && MELEE_WEAPONS.includes(item.itemType));
        }
    ...
    hasRangedWeaponWithAmmo(inventory) {
        var _a, _b;
