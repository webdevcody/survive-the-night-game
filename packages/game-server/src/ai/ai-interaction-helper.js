...
export class AIInteractionHelper {
     ...
     */
    static tryPickupItem(player, entity, targetingSystem, stateName) {
        ...
        if (!targetingSystem.canPickUpItem(inventory, itemType)) {
            ...
            console.error(`[AI ERROR] ${player.getDisplayName()}${statePrefix} attempted to pick up ${itemType} but CANNOT - inventory full and not stackable. This is a targeting logic error! Inventory: ${this.formatInventory(inventory)}`);
     ...
     * @returns InteractionResult indicating success or failure reason
     */
    static tryInteractWithBarrel(player, entity) {
        // Check if entity exists and is not marked for removal
        if (!entity || entity.isMarkedForRemoval()) {
            return { success: false, reason: "entity_invalid" };
        }
     ...
     */
    static canPickupItem(player, entity, targetingSystem) {
        if (!entity || entity.isMarkedForRemoval()) {
            return false;
        }
        if (!entity.hasExt(Interactive)) {
     ...
     */
    static formatInventory(inventory) {
        return inventory.map(item => (item === null || item === void 0 ? void 0 : item.itemType) || "empty").join(", ");
    }
}
