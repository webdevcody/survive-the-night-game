...
import { AIDecision, AIDecisionEngine } from "./ai-decision-engine";
...
import { EngageStateHandler, RetreatStateHandler, LootStateHandler, HuntStateHandler, ExploreStateHandler, } from "./states";
import { AIInteractionHelper } from "./ai-interaction-helper";
import { getEffectiveShootingRange, getMeleeAttackRange, getMeleeRangeWithBuffer, calculateRetreatPosition, equipMeleeWeaponForCrate, equipBagSlotViaLoadout, calculateAimAngle, angleToDirection, aimAtTarget, } from "./ai-utils";
...
export class AIController {
    constructor(player, gameManagers) {
        // Timer management
        this.timerManager = new AITimerManager();
        // Current targets
        ...
        this.currentEnhancedThreatInfo = null;
        this.isInCombat = false;
        // Combat readiness tracking
        this.currentSupplyStatus = null;
        this.player = player;
     ...
     */
    onDamaged(attackerId, entityType, damage) {
        var _a, _b, _c;
        // Record the damage
        ...
        const currentTargetId = (_c = (_b = (_a = this.combatTarget) === null || _a === void 0 ? void 0 : _a.entity) === null || _b === void 0 ? void 0 : _b.getId()) !== null && _c !== void 0 ? _c : null;
    ...
    getCurrentWeaponType() {
        ...
        return activeItem === null || activeItem === void 0 ? void 0 : activeItem.itemType;
    }
    /**
     ...
     */
    checkAndAttackMeleeRangeEnemies(input, playerPos) {
        var _a;
        ...
        const hasMeleeWeapon = inventory.some((item) => item && (item.itemType === "knife" || item.itemType === "baseball_bat"));
        ...
        const nearbyEntities = entityManager.getNearbyEntities(playerPos, meleeRange + 20);
        ...
        for (const otherPlayer of players) {
            if (otherPlayer.getId() === this.player.getId())
                continue;
        ...
        if (closestEnemy && closestPos) {
            ...
            if (!this.combatTarget || ((_a = this.combatTarget.entity) === null || _a === void 0 ? void 0 : _a.getId()) !== closestEnemy.getId()) {
                this.combatTarget = {
                    type: "enemy",
                    entity: closestEnemy,
                    position: closestPos,
    ...
    update(deltaTime) {
        ...
        const enhancedThreatInfo = this.targetingSystem.getEnhancedThreatInfo(this.player, damageHistory, weaponType);
        ...
        const shouldRetarget = this.forceRetarget ||
        ...
        if (shouldRetarget && this.timerManager.reactionTimer <= 0) {
            ...
            if (currentState === AIState.RETREAT && !enhancedThreatInfo.hasImmediateThreat) {
                ...
                if (enhancedThreatInfo.immediateThreat) {
                    this.combatTarget = this.targetingSystem.findBestEnemy(this.player, damageHistory, weaponType);
                }
        ...
        if ((_a = this.combatTarget) === null || _a === void 0 ? void 0 : _a.entity) {
        ...
        if (this.stateMachine.getCurrentState() === AIState.ENGAGE ||
        ...
        if (AI_CONFIG.DEBUG_SHOW_AI_STATE) {
            ...
            this.player.serialized.set("aiState", state);
        }
    }
    /**
    ...
    updateZombieAI(deltaTime) {
        ...
        for (const otherPlayer of players) {
            ...
            if (otherPlayer.isDead())
                continue;
            if (otherPlayer.isZombie())
                continue; // Don't target other zombies
            const otherPos = otherPlayer.getCenterPosition();
            const dist = distance(playerPos, otherPos);
        ...
        if (closestPlayer) {
            ...
            if (this.timerManager.pathRecalcTimer >= AI_CONFIG.PATH_RECALC_INTERVAL * 2 ||
                !this.movementController.getCurrentWaypoint()) {
                this.timerManager.pathRecalcTimer = 0;
                // Pick a random direction to wander
                const randomAngle = Math.random() * Math.PI * 2;
                const wanderDist = 200 + Math.random() * 200;
                const wanderTarget = new Vector2(playerPos.x + Math.cos(randomAngle) * wanderDist, playerPos.y + Math.sin(randomAngle) * wanderDist);
        ...
        if (AI_CONFIG.DEBUG_SHOW_AI_STATE) {
            this.player.serialized.set("aiState", "ZOMBIE_HUNT");
        }
    }
    /**
    ...
    checkIfStuck() {
        ...
        this.movementController.checkIfStuck(currentPos, AI_CONFIG.STUCK_DISTANCE_THRESHOLD, AI_CONFIG.MAX_TARGET_ATTEMPTS, () => {
            ...
            if (item) {
                ...
                if (removed) {
                    ...
                    if (droppedEntity && droppedEntity.hasExt(Positionable)) {
                        ...
                        const dropPos = new Vector2(playerPos.x + (Math.random() - 0.5) * 32, playerPos.y + (Math.random() - 0.5) * 32);
    ...
    checkAndEscapeBearTrap() {
        ...
        for (const bearTrap of bearTraps) {
            ...
            const snaredEntityId = (_a = bearTrap.serialized) === null || _a === void 0 ? void 0 : _a.get("snaredEntityId");
     ...
     */
    makeDecisions(threatInfo, deltaTime) {
        ...
        const decision = this.stateMachine.updateWithDecisionEngine(this.player, threatInfo, hasLootTarget, deltaTime);
     ...
     */
    makeEnhancedDecisions(enhancedThreatInfo, deltaTime) {
        const hasLootTarget = this.targetingSystem.findBestLootTarget(this.player) !== null;
        ...
        const decision = AIDecisionEngine.makeEnhancedDecision(this.player, enhancedThreatInfo, this.currentSupplyStatus, currentState, stateTimer, isAllInMode, hasLootTarget);
     ...
     */
    selectAppropriateWeapon(state) {
        const inventory = this.player.getInventory();
        const currentItem = this.player.activeItem;
        const currentItemType = currentItem === null || currentItem === void 0 ? void 0 : currentItem.itemType;
        if (state === AIState.ENGAGE) {
        ...
        switch (state) {
            ...
            case AIState.LOOT:
                ...
                if (!hasGoodWeapon) {
                    ...
                    const specialBiome = this.targetingSystem.findNearestSpecialBiome(this.player, exploredBiomes);
                    if (specialBiome) {
    ...
    recalculatePath() {
        ...
        if (state === AIState.ENGAGE && this.combatTarget) {
            const targetPos = ((_a = this.combatTarget.entity) === null || _a === void 0 ? void 0 : _a.hasExt(Positionable))
        ...
        if ((_b = this.currentTarget.entity) === null || _b === void 0 ? void 0 : _b.hasExt(Positionable)) {
    ...
    buildStateContext() {
        return {
            ...
            shouldSprint: (isUrgent) => this.shouldSprint(isUrgent),
            calculateAimAngle: (source, target) => this.calculateAimAngle(source, target),
            angleToDirection: (angle) => this.angleToDirection(angle),
            findWalkableWaypoint: (playerPos, targetPos) => this.findWalkableWaypoint(playerPos, targetPos),
            calculateRetreatPosition: (playerPos, enemyPos, dist) => calculateRetreatPosition(playerPos, enemyPos, dist),
            moveTowardWaypoint: (input, playerPos) => this.moveTowardWaypoint(input, playerPos),
     ...
     */
    checkOpportunisticActions(input, playerPos) {
        const state = this.stateMachine.getCurrentState();
        ...
        if (opportunisticItem && opportunisticItem.distance !== undefined) {
            ...
            if (opportunisticItem.distance <= AI_CONFIG.INTERACT_RADIUS &&
                ...
                const result = AIInteractionHelper.tryPickupItem(this.player, opportunisticItem.entity, this.targetingSystem, "OPPORTUNISTIC");
                if (result.success) {
                    this.timerManager.interactTimer = 0;
                    return true;
                }
    ...
    generateInput() {
        ...
        if (input.dx === 0 && input.dy === 0 && !input.fire) {
            ...
            if (!waypoint && this.currentTarget) {
                const newWaypoint = this.pathfinder.pathTowardsAvoidingToxic(playerPos, this.currentTarget.position);
     ...
     */
    findWalkableWaypoint(playerPos, targetPos) {
     ...
     */
    handleMeleeCombat(input, playerPos, enemyPos, dist) {
        const healthPercent = this.player.getHealth() / this.player.getMaxHealth();
        ...
        switch (kitePhase) {
            ...
            case KitePhase.RETREAT:
                ...
                const retreatTarget = calculateRetreatPosition(playerPos, enemyPos, AI_CONFIG.KITE_SAFE_DISTANCE);
     ...
     */
    handleEscapeBehavior(input, playerPos, enemyPos) {
        var _a;
        ...
        const safestDir = (_a = this.currentEnhancedThreatInfo) === null || _a === void 0 ? void 0 : _a.safestRetreatDirection;
        let escapeTarget;
        if (safestDir) {
     ...
     */
    handleDisengageBehavior(input, playerPos, enemyPos, dist) {
        var _a, _b;
        ...
        if (dist <= AI_CONFIG.IMMEDIATE_THREAT_RADIUS ||
            ((_a = this.currentEnhancedThreatInfo) === null || _a === void 0 ? void 0 : _a.hasImmediateThreat)) {
        ...
        const safestDir = (_b = this.currentEnhancedThreatInfo) === null || _b === void 0 ? void 0 : _b.safestRetreatDirection;
        let disengageTarget;
        if (safestDir) {
            disengageTarget = new Vector2(playerPos.x + safestDir.x * 150, playerPos.y + safestDir.y * 150);
        }
        else {
     ...
     * Uses pathfinding to navigate around obstacles
     */
    handleKitingRetreat(input, playerPos, enemyPos, dist) {
        var _a;
        // Get safest retreat direction from enhanced threat info
        const retreatDir = (_a = this.currentEnhancedThreatInfo) === null || _a === void 0 ? void 0 : _a.safestRetreatDirection;
        // Calculate retreat target position using safest direction
        let retreatTarget;
        if (retreatDir) {
            retreatTarget = new Vector2(playerPos.x + retreatDir.x * 150, playerPos.y + retreatDir.y * 150);
        }
        else {
        ...
        if (hasRanged) {
            ...
            const activeItem = this.player.activeItem;
            const weaponType = (activeItem === null || activeItem === void 0 ? void 0 : activeItem.itemType) || "pistol";
     ...
     */
    handleRetreatBehavior(input, playerPos) {
        ...
        if (!hasBandage) {
            ...
            if (bandageTarget &&
                ...
                if (bandageTarget.distance <= AI_CONFIG.INTERACT_RADIUS &&
                    ...
                    this.timerManager.interactTimer = 0;
                    if (!bandageTarget.entity.isMarkedForRemoval() &&
                    ...
                    if (!this.currentTarget ||
                        ...
                        ((_a = this.currentTarget.entity) === null || _a === void 0 ? void 0 : _a.getId()) !== ((_b = bandageTarget.entity) === null || _b === void 0 ? void 0 : _b.getId())) {
        ...
        if (!movedToWaypoint) {
            ...
            if (!this.movementController.getCurrentWaypoint()) {
                const safestDir = (_c = this.currentEnhancedThreatInfo) === null || _c === void 0 ? void 0 : _c.safestRetreatDirection;
                if (safestDir) {
                    // Calculate retreat target away from threats
                    const retreatTarget = new Vector2(playerPos.x + safestDir.x * 300, // Longer retreat distance
     ...
     */
    findRandomWalkableDirection(input, playerPos) {
        // Try multiple random directions until we find one with a valid path
        for (let attempt = 0; attempt < 12; attempt++) {
            const randomAngle = Math.random() * Math.PI * 2;
            const randomTarget = new Vector2(playerPos.x + Math.cos(randomAngle) * 200, playerPos.y + Math.sin(randomAngle) * 200);
            // Always use pathfinding - find walkable waypoint
        ...
        const randomAngle = Math.random() * Math.PI * 2;
        const vel = velocityTowards(playerPos, new Vector2(playerPos.x + Math.cos(randomAngle) * 100, playerPos.y + Math.sin(randomAngle) * 100));
     ...
     */
    handleLootBehavior(input, playerPos) {
        if (!this.currentTarget) {
        ...
        if (dist <= AI_CONFIG.INTERACT_RADIUS) {
            ...
            if (this.timerManager.interactTimer >= AI_CONFIG.INTERACT_COOLDOWN) {
                ...
                if (this.currentTarget.type === "crate") {
                    ...
                    AIInteractionHelper.tryPickupItem(this.player, this.currentTarget.entity, this.targetingSystem, "LOOT_LEGACY");
                    this.currentTarget = null;
                }
            }
     ...
     */
    handleHuntBehavior(input, playerPos) {
        var _a;
        // If we have a player target in range, shoot at them
        if (((_a = this.currentTarget) === null || _a === void 0 ? void 0 : _a.type) === "player" && this.currentTarget.entity) {
     ...
     */
    handleExploreBehavior(input, playerPos) {
        this.moveTowardWaypoint(input, playerPos);
     ...
     */
    moveTowardWaypoint(input, playerPos) {
        const waypoint = this.movementController.getCurrentWaypoint();
        if (!waypoint)
            return false;
...
// Stamina management
AIController.STAMINA_RESERVE_THRESHOLD = 0.3; // Don't sprint below 30% stamina unless urgent
AIController.STAMINA_CRITICAL_THRESHOLD = 0.1; // Below 10%, never sprint unless escaping
