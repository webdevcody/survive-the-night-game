...
export class SentryGunLevel3Client extends ClientEntity implements Renderable {
  ...
  private getHealth(): number {
    if (!this.hasExt(ClientDestructible)) {
      console.warn(`SentryGunLevel3 ${this.getId()} does not have destructible extension`);
      return 0;
    }
