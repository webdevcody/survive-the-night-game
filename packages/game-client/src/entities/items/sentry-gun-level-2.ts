...
export class SentryGunLevel2Client extends ClientEntity implements Renderable {
  ...
  private getHealth(): number {
    if (!this.hasExt(ClientDestructible)) {
      console.warn(`SentryGunLevel2 ${this.getId()} does not have destructible extension`);
      return 0;
    }
